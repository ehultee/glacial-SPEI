...
# -*- coding: utf-8 -*-
"""
Compare precip scaling with original
Created on Mon Feb 28 08:30:51 2022
...
fpath2 = './data/SPEI_Files/precip_scaled/'
...
cases = ['NRunoff', 'WRunoff', 'PrecScal', 'diff'] # inclusion of glacier runoff
...
for m in modelnames:
    ...
    prec_scaled_f_m = fpath2+'PrecScal_{}_{}_{}.txt'.format(integration_times[0], m, scenarios[0])
    SPEI_by_model[m]['NRunoff'] = np.loadtxt(norunoff_f_m)
    SPEI_by_model[m]['WRunoff'] = np.loadtxt(wrunoff_f_m)
    SPEI_by_model[m]['PrecScal'] = np.loadtxt(prec_scaled_f_m)
...
for example_b, example_r, example_pg, ax in zip(example_basins, example_regions, example_pgs, (ax1,ax2,ax3,ax4)):
    ...
    r_p = gSPEI.basin_ensemble_mean(SPEI_by_basin, example_b, 'PrecScal').rolling(window=12*30).mean()
    ...
    rm_p = SPEI_by_basin[example_b]['PrecScal'].rolling(window=12*30, axis=0).mean()
    rm_q1_p = rm_p.quantile(q=0.25, axis=1, interpolation='lower')
    rm_q3_p = rm_p.quantile(q=0.75, axis=1, interpolation='higher')
    ...
    ax.plot(yrs, r_p, 'gray', linewidth=2.0, ls=':')
    ax.plot(yrs, rm_q1_p, 'gray', ls='-.')
    ax.plot(yrs, rm_q3_p, 'gray', ls='-.')
    ...
    ax.fill_between(yrs, rm_q1_p, rm_q3_p, color=color_precip, alpha=0.3)
